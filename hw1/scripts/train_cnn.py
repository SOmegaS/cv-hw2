#!/usr/bin/env python3
"""
Скрипт для обучения SimpleCNN модели
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train SimpleCNN on CIFAR-10 subset')
...
def main():
    ...
    setup_logging()
    logging.info('='*60)
    logging.info('Training SimpleCNN on CIFAR-10 subset')
    logging.info('='*60)
    ...
    train_loader, val_loader, test_loader = get_cifar10_dataloaders(
        ...
        input_size=32,  # CNN работает с 32x32
    ...
    # Создаем модель
    logging.info('Creating SimpleCNN model...')
    model = SimpleCNN(num_classes=len(CLASS_NAMES), dropout=args.dropout)
    ...
    if not args.skip_sanity_check:
        ...
        sanity_loader = sanity_check_overfit_batch(
            model, device, num_samples=32, input_size=32, use_imagenet_stats=False
        )
        
        # Создаем копию модели для sanity check
        sanity_model = SimpleCNN(num_classes=len(CLASS_NAMES), dropout=0.0)  # без dropout для лучшего overfitting
        sanity_model = sanity_model.to(device)
        
        passed = sanity_check_overfit(sanity_model, device, sanity_loader, num_epochs=50)
    ...
    trainer.load_checkpoint('cnn_best.pth')
    
    # Оценка
    from src.evaluate import evaluate_model, save_detailed_results
    
    results = evaluate_model(model, test_loader, device, CLASS_NAMES)
    save_detailed_results(results, 'cnn', CLASS_NAMES, 'results')
